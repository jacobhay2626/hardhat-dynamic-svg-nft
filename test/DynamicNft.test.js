const { assert, equal, expect } = require("chai")
const { network, ethers, deployments } = require("hardhat")
const { isCallTrace } = require("hardhat/internal/hardhat-network/stack-traces/message-trace")
const { developmentChains, networkConfig } = require("../helper-hardhat-config")

const lowSVGImageURI =
    "data:image/svg+xml;base64,"

const highSVGImageURI =
    "data:image/svg+xml;base64,"

const highTokenUri =
    "data:application/json;base64,"

const lowTokenUri =
    "data:application/json;base64,"

!developmentChains.includes(network.name)
    ? describe.skip
    : describe("Dynamic SVG NFT unit tests", function () {
          // beforeEach: transaction so need to get signers, get deployer, deploy mock and contract using the tags
          // get both contracts the mock and the dynamic svg

          beforeEach(async () => {
              const accounts = await ethers.getSigners()
              deployer = accounts[0]
              await deployments.fixture(["mocks", "dynamicsvg"])
              dynamicSvgNft = await ethers.getContract("DynamicSvgNft")
              mockV2Aggregator = await ethers.getContract("MockV3Aggregator")
          })

          // describe constructor: Does it initialise everything correctly?
          // the lowSvg should equal the SvgLowImageURI.
          // price feed should equal the address of the mock.

          describe("constructor", () => {
              it("initialises correctly", async () => {
                  const lowSvg = await dynamicSvgNft.getLowSvg()
                  const highSvg = await dynamicSvgNft.getHighSvg()
                  const priceFeed = await dynamicSvgNft.getPriceFeed()
                  assert.equal(lowSvg, lowSVGImageURI)
                  assert.equal(highSvg, highSVGImageURI)
                  assert.equal(priceFeed, mockV2Aggregator.address)
              })
          })

          // mintNFT: want to check an event is emitted and an NFT is created.
          // mintNFT takes a high value parameter. first define a highValue const variable.
          // Now we have called the function the token counter should equal 1 (0+1)
          // Also, as we have used the high value we should have the token URI equal to the highValueTokenURI
          // we then need to make sure that when the value sent is below the high value
          // that the token uri is reduced to the low value one (frown)

          describe("mintNFT", function () {
              it("emits an event and creates an NFT", async () => {
                  const highValue = ethers.utils.parseEther("1")
                  await expect(dynamicSvgNft.mintNft(highValue)).to.emit(
                      dynamicSvgNft,
                      "createdNFT"
                  )
                  const tokenCounter = await dynamicSvgNft.getTokenCounter()
                  assert.equal(tokenCounter.toString(), "1")
                  const tokenURI = await dynamicSvgNft.tokenURI(0)
                  assert.equal(tokenURI, highTokenUri)
              })

              it("shifts the tokenURI to lower when the value does not much the high value", async () => {
                  const highValue = ethers.utils.parseEther("1000000")
                  const txResponse = await dynamicSvgNft.mintNft(highValue)
                  await txResponse.wait(1)
                  const tokenURI = await dynamicSvgNft.tokenURI(0)
                  assert.equal(tokenURI, lowTokenUri)
              })
          })
      })
